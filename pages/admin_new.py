...
with st.sidebar:
    ...
    for tab_name, icon in admin_tabs:
        ...
        if st.button(f"{icon} {tab_name}", key=f"sidebar_nav_{tab_name}", use_container_width=True, 
...
if selected_tab == "Create Encoder Account":
        ...
        for username, user_data in filtered_users.items():
            ...
            with st.expander(f"👤 {display_name} ({username}) - Created: {created_date}"):
                ...
                with col3:
                    ...
                    if st.button("🗑️ Delete Account", key=f"delete_{username}", type="secondary"):
                        st.session_state[f"confirm_delete_{username}"] = True
                    
                    # Confirmation for deletion
                    if st.session_state.get(f"confirm_delete_{username}", False):
                        st.warning("⚠️ Are you sure you want to delete this account?")
    ...
    with col1:
        ...
        st.info("ℹ️ Admin credentials are stored securely in Streamlit secrets")
    ...
    with col2:
        ...
        if backup_data:
            ...
            if selected_user and st.button("♻️ Restore User"):
